...
void first_pass() {
  ...
  for (int i = 0; i < lastop; i++){
    ...
    switch(op[i].opcode)
    {
      ...
      case VARY:
        if(!frames){
          printf("Vary command found but Frames has not been set. Exiting...\n");
          exit(0);
        }
        else if(!basename){
          strcpy(name, "animation");
          printf("Name not found. Using default name...\n");
        }
        break;
    }
  }
...
struct vary_node ** second_pass() {
  ...
  double start_val, end_val, change_val;
  char name[128];
  ...
  for (int i=0;i<lastop;i++) {
    ...
    switch (op[i].opcode)
      ...
      case VARY:
        ...
        change_val = (end_val - start_val) / (end_frame - start_frame);

        for (int j = start_frame; j <= end_frame; j++){
          curr = malloc(sizeof(struct vary_node));
          strcpy(curr->name, name);
          curr->value = start_val + change_val * (j - start_frame);
          curr->next = knobs[j];
          knobs[j] = curr;
        }
        break;
